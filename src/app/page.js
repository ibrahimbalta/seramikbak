...
export default function Home() {
  ...
  // Preloader removed to show main catalog instantly
      ...
      <div className="content-container">
        ...
        {activeTab === 'search' && (
          <div className="search-portal animate-fade-in">
            {/* Elegant Minimalist Search Bar at the Top */}
            <div className="catalog-search-bar-row" style={{ marginBottom: '24px' }}>
              <form onSubmit={handleSearchSubmit} className="catalog-search-form" style={{ position: 'relative', width: '100%' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', background: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '6px 12px', boxShadow: '0 4px 12px rgba(0,0,0,0.02)', transition: 'all 0.2s' }} className="catalog-search-input-wrapper">
                  <SearchIcon size={18} style={{ color: 'var(--text-secondary)' }} />
                  <input 
                    type="text" 
                    placeholder="Marka, ürün adı, kod, renk, ebat, yüzey, koleksiyon ara..." 
                    value={searchQuery}
                    onChange={handleSearchChange}
                    onFocus={() => { if (searchQuery.trim().length > 1) setShowSuggestions(true); }}
                    onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
                    style={{ flexGrow: 1, border: 'none', outline: 'none', fontSize: '0.85rem', color: 'var(--text-primary)', padding: '6px 0' }}
                  />
                  
                  {/* Visual Search upload button inside search bar */}
                  <div className="search-camera-trigger" title="Görsel ile Arama Yap" style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', position: 'relative' }}>
                    <ImageIcon size={18} style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} />
                    <input 
                      type="file" 
                      accept="image/*" 
                  ...
                  <button type="submit" style={{ background: 'var(--accent-gold)', color: '#ffffff', border: 'none', borderRadius: '8px', padding: '6px 16px', fontSize: '0.8rem', fontWeight: '600', cursor: 'pointer', transition: 'background 0.2s' }}>
                    Ara
                  </button>
                </div>
                    ...
                    {searchSuggestions.map(p => (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                          <span style={{ fontSize: '0.8rem', fontWeight: '650', color: 'var(--text-primary)' }}>{p.name}</span>
                          <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)' }}>{p.brand?.name || 'Seramik'} • {p.width}x{p.height} cm</span>
                        </div>
                      </div>
                    ))}
                  </div>
