...
export default function UyelikPage() {
  ...
  // Render official Google Identity Services buttons
  const renderGoogleButtons = useCallback(() => {
    ...
    try {
      ...
      // Render official register button
  ...
  // Check if window.google is already loaded (from head script)
  useEffect(() => {
    if (typeof window !== 'undefined') {
  ...
  // Whenever Google becomes ready or tab changes, render buttons
  useEffect(() => {
    if (googleReady) {
  ...
  }, [googleReady, authTab, renderGoogleButtons]);

  // Fallback click handler if button is clicked before renderButton finishes
    ...
    if (window.google?.accounts?.id) {
      ...
      const waiter = setInterval(() => {
        ...
        if (window.google?.accounts?.id) {
          ...
          clearInterval(waiter);
          setGoogleLoading(false);
          setError('Google servisine şu anda ulaşılamıyor. Lütfen sayfayı yenileyip tekrar deneyin.');
        }
      }, 150);
    }
  };
        ...
        <div className="uyelik-floating-card">
          ...
          {authTab === 'forgot' ? (
            ...
            <form onSubmit={handleLoginSubmit} className="auth-form">
              ...
              <div style={{ width: '100%', minHeight: '44px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div 
                  ref={googleLoginBtnRef} 
                  style={{ display: googleReady ? 'flex' : 'none', justifyContent: 'center', width: '100%' }}
                />
                {!googleReady && (
                  <button 
                    type="button" 
