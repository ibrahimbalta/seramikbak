...
async function main() {
  ...
  if (!brand) {
    console.error('Bien Seramik brand not found in DB!');
    process.exit(1);
  }

  console.log(`Bien Seramik Brand Found: ${brand.name} (ID: ${brand.id})`);
  
  const total = await prisma.product.count({ where: { brandId: brand.id } });
  console.log('Total Bien Seramik products in DB:', total);
  
  const withRealImages = await prisma.product.count({
    where: { brandId: brand.id, imageUrl: { startsWith: 'https://' } }
  });
  console.log('Bien Seramik products with real image URLs (starting with https):', withRealImages);
  
  const withMockCodes = await prisma.product.count({
    where: { brandId: brand.id, code: { startsWith: 'BIEN-' } }
  });
  console.log('Bien Seramik products with mock codes (BIEN-):', withMockCodes);
  ...
  console.log('Samples of Bien Seramik products in DB:', JSON.stringify(samples, null, 2));
}
