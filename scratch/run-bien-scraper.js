    ...
    try {
      const response = await fetch(url, {
        headers: {
          ...
          'Referer': 'https://www.bienseramik.com.tr/'
        },
...
async function main() {
  console.log('======================================================');
  console.log('           Bien Seramik Ürün Kazıyıcı Script          ');
  console.log('======================================================\n');

  // 1. Bien Seramik Markasını bul
  const brand = await prisma.brand.findFirst({
    where: { name: 'Bien Seramik' }
  });

  if (!brand) {
    console.error('[Hata] Veritabanında "Bien Seramik" markası bulunamadı! Lütfen önce veritabanını seed edin.');
    process.exit(1);
  }
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    // Display Name
    const displayName = `Bien ${name} ${size}`;
    ...
    parsedProducts.push({
      ...
      imageUrl: cleanImg,
      textureUrl: cleanImg,
      sourceUrl: href.startsWith('http') ? href : `https://www.bienseramik.com.tr/${href}`
    });
  }
  ...
  // 4. Eski mock verileri temizle (BIEN- ön ekiyle başlayan mocklar)
  console.log(`\n[Veritabanı] Eski mock (BIEN-*) Bien Seramik ürünleri temizleniyor...`);
  try {
    ...
    console.log(`[Veritabanı] ${deleteResult.count} adet eski mock Bien Seramik ürünü silindi.`);
  } catch (err) {
    console.error('[Veritabanı Hata] Eski mock ürünler silinemedi:', err.message);
  }

  // 5. Veritabanına kaydet (upsert)
  console.log(`\n[Veritabanı] ${parsedProducts.length} adet Bien Seramik ürününün veritabanına kaydedilmesi başlatılıyor...`);
  let addedCount = 0;
  let errorCount = 0;
  ...
  for (const prod of parsedProducts) {
    try {
      ...
      if (addedCount % 50 === 0 || addedCount === parsedProducts.length) {
        console.log(`[Veritabanı İlerleme] ${addedCount}/${parsedProducts.length} Bien Seramik ürünü upsert edildi.`);
      }
    } catch (err) {
      errorCount++;
  ...
  console.log(`\n[Sistem] ✅ Bien Seramik Entegrasyonu Başarıyla Tamamlandı!`);
