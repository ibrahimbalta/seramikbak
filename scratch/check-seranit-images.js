...
async function main() {
  ...
  if (!brand) {
    console.error('Seranit brand not found in DB!');
    process.exit(1);
  }

  console.log(`Seranit Brand Found: ${brand.name} (ID: ${brand.id})`);
  
  const total = await prisma.product.count({ where: { brandId: brand.id } });
  console.log('Total Seranit products in DB:', total);
  ...
  console.log('Samples of Seranit products in DB:', JSON.stringify(samples, null, 2));
}
