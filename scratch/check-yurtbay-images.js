...
async function main() {
  ...
  if (!brand) {
    console.error('Yurtbay Seramik brand not found in DB!');
    process.exit(1);
  }

  console.log(`Yurtbay Seramik Brand Found: ${brand.name} (ID: ${brand.id})`);
  
  const total = await prisma.product.count({ where: { brandId: brand.id } });
  console.log('Total Yurtbay Seramik products in DB:', total);
  
  const withRealImages = await prisma.product.count({
    where: { brandId: brand.id, imageUrl: { startsWith: 'https://' } }
  });
  console.log('Yurtbay Seramik products with real image URLs (starting with https):', withRealImages);
  
  const withMockCodes = await prisma.product.count({
    where: { brandId: brand.id, code: { startsWith: 'YURT-' } }
  });
  console.log('Yurtbay Seramik products with mock codes (YURT-):', withMockCodes);
  ...
  console.log('Samples of Yurtbay Seramik products in DB:', JSON.stringify(samples, null, 2));
}
