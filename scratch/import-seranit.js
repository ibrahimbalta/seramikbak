...
async function main() {
  console.log('======================================================');
  console.log('         Seranit Ürün İthalatçı (Prisma)              ');
  console.log('======================================================\n');

  const inputPath = path.join(__dirname, 'seranit-products.json');
  ...
  let brand = await prisma.brand.findFirst({
    where: { name: { contains: 'Seranit' } }
  });

  if (!brand) {
    console.log('[Veritabanı] "Seranit" markası bulunamadı. Yeni marka oluşturuluyor...');
    brand = await prisma.brand.create({
      data: {
        name: 'Seranit',
        logoUrl: '/logos/seranit.png'
      }
    });
  }
  console.log(`Hedef Marka: ${brand.name} (ID: ${brand.id})`);

  // 2. Eski mock verileri temizle
  console.log('[Veritabanı] Eski Seranit ürünleri temizleniyor...');
  try {
    ...
    console.log(`[Veritabanı] ${deleteResult.count} adet eski Seranit ürünü silindi.`);
  } catch (err) {
    console.error('[Veritabanı Hata] Eski ürünler silinemedi:', err.message);
  }

  // 3. Ürünleri Veritabanına Aktar (Prisma Upsert)
  ...
  for (const prod of products) {
    try {
      ...
      // Her sorgu arasına 20ms bekleme ekleyerek veritabanını koruyalım
      await sleep(20);
    } catch (err) {
      errorCount++;
  ...
  console.log(`\n[Sistem] ✅ Seranit İçe Aktarımı Tamamlandı!`);
