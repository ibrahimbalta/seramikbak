...
async function main() {
  ...
  if (!brand) {
    console.error('Qua Granite brand not found in DB!');
    process.exit(1);
  }

  console.log(`Qua Granite Brand Found: ${brand.name} (ID: ${brand.id})`);
  
  const total = await prisma.product.count({ where: { brandId: brand.id } });
  console.log('Total Qua Granite products in DB:', total);
  
  const withRealImages = await prisma.product.count({
    where: { brandId: brand.id, imageUrl: { startsWith: 'https://' } }
  });
  console.log('Qua Granite products with real image URLs (starting with https):', withRealImages);
  
  const withMockCodes = await prisma.product.count({
    where: { brandId: brand.id, code: { startsWith: 'QUA-' } }
  });
  console.log('Qua Granite products with mock codes (QUA-):', withMockCodes);
  ...
  console.log('Samples of Qua Granite products in DB:', JSON.stringify(samples, null, 2));
}
