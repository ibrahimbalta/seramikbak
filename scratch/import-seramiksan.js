...
async function main() {
  console.log('======================================================');
  console.log('         Seramiksan Ürün İthalatçı (Prisma)            ');
  console.log('======================================================\n');

  const inputPath = path.join(__dirname, 'seramiksan-products.json');
  ...
  if (!brand) {
    console.log('[Veritabanı] "Seramiksan" markası bulunamadı. Yeni marka oluşturuluyor...');
    brand = await prisma.brand.create({
      data: {
        name: 'Seramiksan',
        logoUrl: '/logos/seramiksan.png'
      }
    });
  }
  ...
  console.log('[Veritabanı] Eski Seramiksan ürünleri temizleniyor...');
  try {
    ...
    console.log(`[Veritabanı] ${deleteResult.count} adet eski Seramiksan ürünü silindi.`);
  } catch (err) {
  ...
  for (const prod of products) {
    try {
      ...
      // Veritabanını korumak için 20ms bekleme ekleyelim
      await sleep(20);
    } catch (err) {
      errorCount++;
  ...
  console.log(`\n[Sistem] ✅ Seramiksan İçe Aktarımı Tamamlandı!`);
